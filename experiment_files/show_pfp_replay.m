function show_pfp_replay(aPFP_mat, texts, reward, window, white, masktexture, mask_locs, rect_locs)

% This function executes a replay
%
% Parameters
% ----------
% - data_mat: The aPFP_mat from which a previous game can be deduced.
...
[~, pfp_trials, pfp_runs] = size(aPFP_mat)                                  ; % trials are saved in 2nd dim, runs in 3rd dim
...
for replay_run = 1:pfp_runs
    ...
    % decision
    picked_loc = aPFP_mat(1,replay_trial,replay_run)                        ; % which location was picked
    rt = aPFP_mat(2,replay_trial,replay_run)                                ; % how quickly was it picked
    reward_bool = aPFP_mat(4,replay_trial,replay_run)                       ; % was it rewarded?
